#!/usr/bin/env bash
# GitHub environment detection helper for Odysseus Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _ody_env="${ODYSSEUS_HOME:-$HOME/.odysseus}/.env"; [ -f "$_ody_env" ] && grep -q "^GITHUB_TOKEN=" "$_ody_env" 2>/dev/null; then

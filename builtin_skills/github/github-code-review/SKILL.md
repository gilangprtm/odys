...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _ody_env="${ODYSSEUS_HOME:-$HOME/.odysseus}/.env"; [ -f "$_ody_env" ] && grep -q "^GITHUB_TOKEN=" "$_ody_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ody_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Odysseus\",
    \"comments\": [
...
```bash
source "${ODYSSEUS_HOME:-$HOME/.odysseus}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Odysseus. Code looks clean — good test coverage, no security concerns."

...
metadata:
  odysseus:
    ...
    related_skills: [claude-code, odysseus-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Odysseus terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Odysseus itself, `model.provider: openai-codex` uses Odysseus-managed Codex
OAuth from `~/.odysseus/auth.json` after `odysseus auth add openai-codex`. For the
...
## Odysseus Gateway Caveat

When invoking the Codex CLI from a Odysseus gateway/service context (for example,

...
description: "Operate the Teams meeting summary pipeline via Odysseus CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
author: Odysseus
license: MIT
prerequisites:
...
Everything operator-facing is a `odysseus teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${ODYSSEUS_HOME:-~/.odysseus}/.env`:
...
```bash
odysseus teams-pipeline validate              # config snapshot — run first after any change
odysseus teams-pipeline token-health          # Graph token status
odysseus teams-pipeline token-health --force-refresh   # force a fresh token acquisition
odysseus teams-pipeline list                  # recent meeting jobs
odysseus teams-pipeline list --status failed  # only failed jobs
odysseus teams-pipeline show <job-id>         # full detail of one job
odysseus teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
odysseus teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
odysseus teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
odysseus teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
odysseus teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

odysseus teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
odysseus teams-pipeline delete-subscription <sub-id>
odysseus teams-pipeline maintain-subscriptions            # renew near-expiry ones
odysseus teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `odysseus teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `odysseus cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).

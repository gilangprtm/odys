...
## 5. Odysseus Configuration

### Config location

`$ODYSSEUS_HOME/config.yaml` (defaults to `~/.odysseus/config.yaml` when `ODYSSEUS_HOME` is unset)
...
Restart the Odysseus session for changes to take effect. The MCP connection is

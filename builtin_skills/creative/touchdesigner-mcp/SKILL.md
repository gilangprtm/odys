...
metadata:
  odysseus:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, odysseus-video]
...
```
Odysseus -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${ODYSSEUS_HOME:-$HOME/.odysseus}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Odysseus config (if missing)
...
3. **Restart Odysseus session** to pick up the new MCP server

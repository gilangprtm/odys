                    ...
                    try:
                        ...
                        if summary:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, subject, sender, summary, model, datetime.now(timezone.utc).replace(tzinfo=None).isoformat()))
                            _c.commit()
                            _c.close()
                    ...
                    try:
                        ...
                        if reply:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """, (message_id, account_owner or "", uid.decode() if isinstance(uid, bytes) else str(uid), _folder, reply, model, datetime.now(timezone.utc).replace(tzinfo=None).isoformat()))
                            _c.commit()
                            _c.close()
